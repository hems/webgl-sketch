...
const handleFile = (input, output, opts) => fsP.readFile(input).then(data => {
	const dirname = path.dirname(input).split('src/assets/image/')[1];
...
// 	opts.plugins = opts.use || opts.plugins;
//
// 	const pipe = opts.plugins.length > 0
// 		? promisePipe(opts.plugins)(input) : Promise.resolve(input);
//
// 	return pipe
// 		.then(buf => buf.length < input.length ? buf : input);
// };
